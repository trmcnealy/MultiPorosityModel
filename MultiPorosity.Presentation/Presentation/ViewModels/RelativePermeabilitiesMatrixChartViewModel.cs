...
{
    public class RelativePermeabilitiesMatrixChartViewModel : BindableBase
    {
        ...
        public SelectedData[] SelectedRecords
        {
            ...
            set
            {
                ...
                {
                    _multiPorosityModelService.ActiveProject.SelectedRelativePermeabilityMatrixModels.Clear();

                    for(int i = 0; i < selected.Length; ++i)
                    {
                        _multiPorosityModelService.ActiveProject.SelectedRelativePermeabilityMatrixModels.Add(_multiPorosityModelService.ActiveProject.RelativePermeabilityMatrixModels[selected[i].PointIndex]);
                    }
        ...
        public RelativePermeabilitiesMatrixChartViewModel(MultiPorosityModelService multiPorosityModelService)
        {
                                       ...
                                       PropertyChangedEventArgs e)
        {
            switch(e.PropertyName)
            {
                case "ActiveProject":
                {
                    ...
                    _multiPorosityModelService.ActiveProject.PropertyChanged                                    += OnPropertyChanged;
                    _multiPorosityModelService.ActiveProject.RelativePermeabilityMatrixModels.CollectionChanged -= OnRelativePermeabilityModelsChanged;
                    _multiPorosityModelService.ActiveProject.RelativePermeabilityMatrixModels.CollectionChanged += OnRelativePermeabilityModelsChanged;
                    OnRelativePermeabilityModelsChanged(sender, null);

                    break;
                }
                case "RelativePermeabilityMatrixModels":
                {
                    OnRelativePermeabilityModelsChanged(sender, null);
                                                         ...
                                                         NotifyCollectionChangedEventArgs? e)
        {
            ...
            RelativePermeabilityModel[]? relativePermeabilityModelsSoArray = _multiPorosityModelService.ActiveProject.RelativePermeabilityMatrixModels.Where(m => m.Sg               == 0.0).ToArray();
            RelativePermeabilityModel[]? relativePermeabilityModelsSgArray = _multiPorosityModelService.ActiveProject.RelativePermeabilityMatrixModels.Where(m => m.So == 0.0).ToArray();
