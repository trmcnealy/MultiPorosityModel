...
namespace MultiPorosity.Services.Optimization
{
    ...
    /// <returns></returns>
    public delegate int LM_Function(double[]           a,
                                    double[]           fvec,
                                    LM_Array<double>[] dvec,
                                    object             prv);
...
#if FLOAT_PRECISION
    /* Float precision */
            public const float  MP_MACHEP0
 = 1.19209e-07;
            public const float  MP_DWARF
 = 1.17549e-38;
            public const float  MP_GIANT
 = 3.40282e+38;
            public const float MP_RDWARF
 = 1.3278686946331594e-018;
            public const float MP_RGIANT
 = 1844673472786071600;
#else
        ...
        * ********** */
        ...
        /// </param>
        /// <returns></returns>
        public static int Solve(LM_Function    funct,
                                int            m,
                                int            npar,
                                double[]       xall,
                                LM_Parameter[] pars,
                                LM_Config      config,
                                object         prv,
                                ref LM_Result  result)
        {
            var conf = new LM_Config();
            ...
            if(pars != null)
            {
                for(i = 0; i < npar; i++)
                {
                    if((pars[i].limited[0] != 0 && (xall[i] < pars[i].limits[0])) || (pars[i].limited[1] != 0 && (xall[i] > pars[i].limits[1])))
                    {
                    ...
                    if((pars[i].isFixed != 0) && (pars[i].limited[0] != 0) && (pars[i].limited[1] != 0) && (pars[i].limits[0] >= pars[i].limits[1]))
                    {
                        info = MP_ERR_BOUNDS;
            ...
            if((npar <= 0) || (conf.ftol <= 0) || (conf.xtol <= 0) || (conf.gtol <= 0) || (conf.maxiter < 0) || (conf.stepfactor <= 0))
            {
                info = MP_ERR_PARAM;
            ...
            iflag = LM_FDJac2(funct, m, nfree, ifree, npar, xnew, fvec, fjac, ldfjac, conf.epsfcn, wa4, prv, ref nfev, step, dstep, mpside, qulim, ulim, ddebug, ddrtol, ddatol);
            ...
            LM_QRFac(m, nfree, fjac, ldfjac, 1, ipvt, nfree, wa1, wa2, wa3);
            ...
            if(fnorm != zero)
            {
                ...
                for(j = 0; j < nfree; j++)
                {
                    ...
                    if(wa2[l] != zero)
                    {
                        ...
                        gnorm = LM_DMax1(gnorm, Math.Abs(sum / wa2[l]));
                    }
            ...
            if(conf.douserscale == 0)
            {
                for(j = 0; j < nfree; j++)
                {
                    diag[ifree[j]] = LM_DMax1(diag[ifree[j]], wa2[j]);
                }
            }
             ...
             */
            LM_LMPar(nfree, fjac, ldfjac, ipvt, ifree, diag, qtf, delta, ref par, wa1, wa2, wa3, wa4);
            ...
            else
            {
                ...
                for(j = 0; j < nfree; j++)
                {
                    ...
                    if(lpegged != 0 && (wa1[j] < 0))
                    {
                        wa1[j] = 0;
                    }

                    if(upegged != 0 && (wa1[j] > 0))
                    {
                        wa1[j] = 0;
                    }
                    ...
                    {
                        alpha = LM_DMin1(alpha, (llim[j] - x[j]) / wa1[j]);
                    }

                    if(dwa1 != 0 && qulim[j] != 0 && (x[j] + wa1[j] > ulim[j]))
                    {
                        alpha = LM_DMin1(alpha, (ulim[j] - x[j]) / wa1[j]);
                    }
                }
            ...
            if(ratio <= p25)
            {
                ...
                delta = temp * LM_DMin1(delta, pnorm / p1);
                par   = par  / temp;
            }
            else
            {
            ...
            if((Math.Abs(actred) <= conf.ftol) && (prered <= conf.ftol) && (p5 * ratio <= one) && (info == 2))
            {
                info = MP_OK_BOTH;
            }
            ...
            if(pars != null)
            {
                for(i = 0; i < npar; i++)
                {
                    if((pars[i].limited[0] != 0 && (pars[i].limits[0] == xall[i])) || (pars[i].limited[1] != 0 && (pars[i].limits[1] == xall[i])))
                    {
                        npegged++;
                    }
                }
            ...
            {
                LM_Convar(nfree, fjac, ldfjac, ipvt, conf.covtol, wa2);
            ...
            if(result != null)
            {
                ...
                result.bestnorm =  LM_DMax1(fnorm, fnorm1);
        ...
        *
              **********
        */
        private static int LM_FDJac2(LM_Function funct,
                                     int         m,
                                     int         n,
                                     int[]       ifree,
                                     int         npar,
                                     ...
                                     double[]    ddatol)
        {
            ...
            temp = LM_DMax1(epsfcn, MP_MACHEP0);
            eps  = Math.Sqrt(temp);
            ij   = 0;

            //dvec = (double**)malloc(sizeof(double**) * npar);
            //dvec = new double[npar][];
            dvec = new LM_Array<double>[npar];
            ...
            for(j = 0; j < n; j++)
            {
                ...
                if(dside != null && dside[ifree[j]] == 3 && ddebug[ifree[j]] == 0)
                {
                    ...
                    dvec[ifree[j]]     = new LM_Array<double>(fjac, j * m, m); //fjac + j * m;
            ...
            if(hasAnalyticalDeriv != 0)
            {
                ...
                if(nfev != 0)
                {
                    nfev = nfev + 1; //todo: correct translation from C?
                }
            ...
            if(hasDebugDeriv != 0)
            {
                Debug.Write("FJAC DEBUG BEGIN\n");

                //Debug.Write("#  %10s %10s %10s %10s %10s %10s\n",
                Debug.Write(string.Format("#  {0} {1} {2} {3} {4} {5}\n", "IPNT", "FUNC", "DERIV_U", "DERIV_N", "DIFF_ABS", "DIFF_REL"));
            }
            ...
            if(hasNumericalDeriv != 0)
            {
                for(j = 0; j < n; j++)
                {
                    ...
                    if(debug != 0)
                    {
                        Debug.Write(string.Format("FJAC PARM {0}\n", ifree[j]));
                    }
                    ...
                    if(step != null && step[ifree[j]] > 0)
                    {
                        h = step[ifree[j]];
                    }

                    if(dstep != null && dstep[ifree[j]] > 0)
                    {
                        h = Math.Abs(dstep[ifree[j]] * temp);
                    }
                    ...
                    if((dside != null && dsidei == -1) || (dside != null && dsidei == 0 && qulimited != null && ulimit != null && qulimited[j] != 0 && (temp > ulimit[j] - h)))
                    {
                        h = -h;
                    }
                    ...
                    if(dsidei <= 1)
                    {
                        ...
                        else
                        {
                            ...
                            for(i = 0; i < m; i++, ij++)
                            {
                                ...
                                if((da == 0 && dr == 0 && (fjold != 0 || fjac[ij] != 0)) || ((da != 0 || dr != 0) && (Math.Abs(fjold - fjac[ij]) > da + Math.Abs(fjold) * dr)))
                                {
                                    //Debug.Write("   %10d %10.4g %10.4g %10.4g %10.4g %10.4g\n",
        ...
        private static void LM_QRFac(int      m,
                                     int      n,
                                     double[] a,
                                     int      lda,
                                     int      pivot,
                                     ...
                                     double[] wa)
        {
            ...
            */
            ...
            var aTemp = new LM_Array<double>(a, ij, n);
                ...
                if(jp1 < n)
                {
                    for(k = jp1; k < n; k++)
                    {
                        ...
                        if((pivot != 0) && (rdiag[k] != zero))
                        {
                            ...
                            temp     =  LM_DMax1(zero, one - temp * temp);
        ...
        private static void LM_QRSolv(int      n,
                                      double[] r,
                                      int      ldr,
                                      int[]    ipvt,
                                      double[] diag,
        ...
        private static void LM_LMPar(int        n,
                                     double[]   r,
                                     int        ldr,
                                     int[]      ipvt,
                                     int[]      ifree,
                                     ...
                                     double[]   wa2)
        {
            ...
            if(paru == zero)
            {
                paru = MP_DWARF / LM_DMin1(delta, p1);
            }
           ...
           */
            par = LM_DMax1(par, parl);
            par = LM_DMin1(par, paru);
            ...
            if(par == zero)
            {
                par = LM_DMax1(MP_DWARF, p001 * paru);
            }
            ...
            if(fp > zero)
            {
                parl = LM_DMax1(parl, par);
            }

            if(fp < zero)
            {
                paru = LM_DMin1(paru, par);
            }

            /*
             *	 compute an improved estimate for par.
             */
            par = LM_DMax1(parl, par + parc);
        ...
        private static double MP_ENorm(int              n,
                                       LM_Array<double> x)
        {
            /*
             *     **********
        ...
        private static double LM_DMax1(double a,
                                       double b)
        {
            if(a >= b)
            {
        ...
        private static double LM_DMin1(double a,
                                       double b)
        {
            if(a <= b)
            {
        ...
        private static int LM_Min0(int a,
                                   int b)
        {
            if(a <= b)
            {
        ...
        */
        ...
        private static int LM_Convar(int      n,
                                     double[] r,
                                     int      ldr,
                                     int[]    ipvt,
                                     double   tol,
                                     double[] wa)
        {
            ...
            //                for(i = 0; i < n; i++)
            //                {
            //                    Debug.Write(string.Format("{0} ", r[j*ldr + i]));
            //                }
            //                Debug.Write("\n");
            //            }
            //#endif
